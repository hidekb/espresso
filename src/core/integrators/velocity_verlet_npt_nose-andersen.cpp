    ...
    for (unsigned int j = 0; j < 3; j++) {
      if (!p.is_fixed_along(j)) {
        if (nptiso.geometry & ::nptgeom_dir[j]) {
          ...
          nptiso.p_vel_tpdt[j] += Utils::sqr(p.v()[j]) * p.mass();
        } else {
...
velocity_verlet_npt_propagate_pos(ParticleRangeNPT const &particles,
  ...
  /* adjust \ref NptIsoParameters::nptiso.volume with dt/2; prepare pos-rescaling
   */
  if (this_node == 0) {
   ...
   * readjust \ref NptIsoParameters::nptiso.volume with dt/2; prepare pos- and
   * vel-rescaling
   */
  nptiso.p_epsilon = propagate_thermV_nptiso_dt(npt_iso, nptiso.p_epsilon, nptiso.piston);
  if (this_node == 0) {
    nptiso.volume += nptiso.inv_piston * nptiso.p_epsilon * 0.5 * time_step;
